...
import message6.web.WebContext;
import message6.web.common.CancelableImporterTask;
import message6.web.common.TimePeriod;
//import message6.web.events.Event;
//import message6.web.events.EventManager;
//import message6.web.events.GroupInfo;
//import message6.web.events.ProcessEvent;
import message6.web.util.ReflectionUtil;
...
//    }
    ...
    public
    HashMap cancelUpload(HttpServletRequest request, HttpServletResponse response) throws IOException {
...
//    public
//    HashMap commonPoll(HttpServletRequest request, HttpServletResponse response) throws IOException {
//        try {
//            List list = new ArrayList<>();
//            HashMap map = new HashMap();
//            String group = request.getParameter("group");
//         //   System.out.println("poll---"+group);
//            if (!EventManager.getInstance().groups.containsKey(group)) {
//                map.put("progress",1000);
//                return map;
//            } else {
//                GroupInfo groupInfo = EventManager.getInstance().groups.get(group);
//                map.put("progress",groupInfo.getProcess());
//                map.put("success",groupInfo.getSuccess());
//                map.put("fail",groupInfo.getFail());
//                map.put("title","");
//            }
//
//
//            List<Event> events = EventManager.getInstance().takeAll(group);
//
...
//                }
//
//                ProcessEvent value =(ProcessEvent) list.get(list.size() - 1);
//                map.put("logs",events);
//                map.put("title",value.getTitle());
//                return map;
//
//                //有时候虽然group执行完了，但是日志还没收完，所以要先收日志，这里的逻辑还可以优化
//            } else if (EventManager.getInstance().isGroupEnd(group)) {
//                map.put("progress",1000);
//                return map;
//            } else {
//                map.put("progress", EventManager.getInstance().groups.containsKey(group) ? EventManager.getInstance().groups.get(group).getProcess() : 0);
//            }
//            return map;
//        } catch (Exception e) {
//            logger.error(e.getMessage(), e);
